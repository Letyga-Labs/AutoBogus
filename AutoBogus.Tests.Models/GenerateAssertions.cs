...
{
    private readonly IDictionary<Func<Type, bool>, Func<string, Type, object, string>> Assertions =
        new Dictionary<Func<Type, bool>, Func<string, Type, object, string>>();

    private readonly MethodInfo DefaultValueFactory;
    ...
    {
        return value != null && DateTime.TryParse(value.ToString(), out var result) && result != default
            ? null
            : GetAssertionMessage(path, type, value);
    }
    ...
    {
        return value != null && DateTimeOffset.TryParse(value.ToString(), out var result) && result != default
            ? null
            : GetAssertionMessage(path, type, value);
    }
    ...
    {
        return value != null && Guid.TryParse(value.ToString(), out var result) && result != default
            ? null
            : GetAssertionMessage(path, type, value);
    }
    ...
    {
        return value != null && IPAddress.TryParse(value.ToString(), out var result) && result != default(IPAddress)
            ? null
            : GetAssertionMessage(path, type, value);
    }
