...
internal class Templator<T> where T : class
{
    ...
    private string propertyNameSpaceDelimiter;

    //if false will treat missing as empty
    private bool                                                    treatMissingAsNull = true;
    private List<Func<Type, string, (bool handled, object result)>> typeConverters     = new();
    ...
    /// <summary>
    ///     Creates a set of test data with number of rows matching testItems but only using the headers in the headers list in
    ///     the template
    ///     Properties not in the headers list will use AutoFaker rules
    /// </summary>
    /// <param name="headers">
    ///     List of headers matching testItems. Defines what properties in testItems we will set the value
    ///     for
    /// </param>
